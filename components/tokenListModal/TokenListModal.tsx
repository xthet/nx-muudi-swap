...
export default function TokenListModal({ offMe, tokens, insetTkn, selTkn }:props) {
  ...
  function findInput(val:string){
    ...
    let qTknsArr:gtkn[] = tokens.filter(token=>{
      if(val == ""){return token}
      else if(token.name.toLowerCase() == val.toLowerCase() 
      ...
      <div className="tlm">
        <div className="tlm-box">
          <p className="tlm-title">
            {"Select a token"}
            <FontAwesomeIcon icon={faXmark} className="tlm-x-icon" onClick={()=>{offMe()}}/>
          </p>
          <div className="tlm-search">
            <FontAwesomeIcon icon={faMagnifyingGlass} className="tlm-search-icon"/>
            <input type="text" className="tlm-inpt"
              placeholder="Search token name or paste address" onChange={(e)=>{findInput(e.target.value)}}
            />
          </div>
          <div className="tlm-pop-tkns-wrp">
            {popTokens.popTkns.map((popTkn:gtkn, index)=>{
              return (
                <span key={index} className={`tlm-pop-tkn ${popTkn.name == insetTkn.name && "--tkn-disabled"}`} 
                  onClick={()=>{checkClickedTkn(popTkn)}}
                >
          ...
          <div className="tlm-grp" ref={rootRef}>
            {tknArr.slice(0,offset).map((token, index)=>{
              return (
                <div key={index} className={`tlm-tkn-grp ${token.name == insetTkn.name && "--tkn-disabled"}`} 
                  onClick={()=>{checkClickedTkn(token)}}
                >
